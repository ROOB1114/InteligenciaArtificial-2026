...
class Puzzle8I {
    ...
    public void buscarSolucion(String estadoInicial, String estadoObjetivo) {
        Queue<NodoI> cola = new LinkedList<>();
        HashSet<String> visitados = new HashSet<>();

        // El nodo raíz tiene profundidad 0 (0 transiciones)
        NodoI raiz = new NodoI(estadoInicial, null, 0);
        ...
        while (!cola.isEmpty()) {
            ...
            for (String hijoEstado : hijos) {
                if (!visitados.contains(hijoEstado)) {
                    ...
                    cola.add(new NodoI(hijoEstado, actual, actual.profundidad + 1));
                }
            }
        }
    ...
    private void imprimirCamino(NodoI nodo) {
        LinkedList<NodoI> camino = new LinkedList<>();
        NodoI aux = nodo;
        while (aux != null) {
            camino.addFirst(aux);
            aux = aux.padre;
